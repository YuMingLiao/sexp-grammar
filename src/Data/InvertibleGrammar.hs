...
class (Typeable a, Ord a, Show a) => Taggy a where
  type Original a
  mkTag :: Original a -> a
  mkMismatch :: a -> Mismatch

newtype Tag a = Tag Int
  deriving (Show, Eq, Ord)

data ConstTag a = ConstTag
  deriving (Show, Eq, Ord)

instance (Typeable a) => Taggy (Tag a) where
  type Original (Tag a) = a
  mkTag a = Tag (I# (dataToTag# a))
  mkMismatch (Tag n) = expected . pack . show $ n

instance forall a. (Typeable a) => Taggy (ConstTag a) where
  type Original (ConstTag a) = a
  mkTag _ = ConstTag
  mkMismatch ConstTag = expected . pack . show . typeRep $ Proxy @a
...
data Grammar p a b where
  ...
  Select     :: (Taggy ia, Taggy ib, Original ia ~ a, Original ib ~ b) =>
                Map ia (Grammar p (a :- t) (b :- t))
             -> Map ib (Grammar p (a :- t) (b :- t))
             -> Grammar p (a :- t) (b :- t)
...
select :: (Taggy a, Taggy b) => [(a, b, Grammar p (Original a :- t) (Original b :- t))] -> Grammar p (Original a :- t) (Original b :- t)
select lst =
...
forward (Select mg _)    = \x@(a :- _) ->
  case M.lookup (mkTag a) mg of
